def time_string(hours, minutes, time_format):
    # Formatting minutes to always have two digits
    minutes_str = f"{minutes:02d}"
    
    if time_format == '24':
        # Return in 24-hour format: 'HH:MM'
        return f"{hours:02d}:{minutes_str}"
    elif time_format == '12':
        # For 12-hour format, we need to adjust the hours
        if hours == 0:
            hour12 = 12
            suffix = "am"
        elif 1 <= hours and hours < 12:
            hour12 = hours
            suffix = "am"
        elif hours == 12:
            hour12 = 12
            suffix = "pm"
        else:  # 13 to 23 hours should be pm
            hour12 = hours - 12
            suffix = "pm"
        
        # Return in 12-hour format: 'h:mmam/pm'
        return f"{hour12}:{minutes_str}{suffix}"
    else:
        raise ValueError("Invalid time format. Please use '24' or '12'.")

# Test cases
print(time_string(15, 38, '24'))  # '15:38'
print(time_string(8, 3, '24'))    # '08:03'
print(time_string(0, 5, '24'))    # '00:05'
print(time_string(11, 15, '12'))  # '11:15am'
print(time_string(0, 7, '12'))    # '12:07am'
print(time_string(7, 30, '12'))   # '7:30am'
print(time_string(12, 46, '12'))  # '12:46pm'
print(time_string(13, 10, '12'))  # '1:10pm'
print(time_string(19, 2, '12'))   # '7:02pm'
